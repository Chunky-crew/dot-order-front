...
export default function OrderMenuClient({ tableNumberStr, initialCategories, initialMenuItems }: OrderMenuClientProps) {
  ...
  const [allItems] = useState<MenuItem[]>(initialMenuItems);
  ...
  return (
    <div className="flex flex-col pb-28">
      {/* Category tabs + persistent cart icon */}
      <div className="sticky top-[56px] z-20 bg-background border-b border-border shadow-sm flex items-center">
        <div
          ref={tabsRef}
          className="hide-scrollbar flex overflow-x-auto px-4 gap-1 py-2 flex-1 min-w-0"
        >
          <button
        ...
        <button
          ...
          className="relative flex-shrink-0 mx-2 w-10 h-10 rounded-full flex items-center justify-center hover:bg-muted active:scale-95 transition-all"
          aria-label={`장바구니 보기 (${cart.totalCount}개)`}
        >
          <svg
            className="w-5 h-5 text-maroon-800"
            fill="none"
            stroke="currentColor"
            strokeWidth={1.8}
            viewBox="0 0 24 24"
          ...
          {cart.totalCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 bg-maroon-800 text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 tabular-nums">
